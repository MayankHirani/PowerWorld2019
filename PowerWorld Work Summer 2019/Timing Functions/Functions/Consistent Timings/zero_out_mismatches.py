...
Description: Zeroes out the mismatches of buses
...
# Create a function that the timeit can call that will zero the buses
# whose mismatches are greater than the 
def solve():
	SimAuto.RunScriptCommand("ZeroOutMismatches;")
...
def time_solve(file_name):
	...
	timings = []
	for num in range(2):
		x = timeit.timeit(solve, number=100)
		timings.append(x)
